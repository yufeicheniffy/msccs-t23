...
class twitter_api:
    ...
    def list_from_ids(self, ids):
        ...
        for i in range(1, iterations + 1):
            ...
            for t in tweets:
                c = self.DB.find_category_by_id(t.id)
                list.append({"postID": repr(t.id), "text": t.text, "categories": c})
        ...
        for t in tweets:
            c = self.DB.find_category_by_id(t.id)
            list.append({"postID": repr(t.id), "text": t.text, "categories": c})
        print("twitter_api.list length="+repr(len(list)))
    ...
    def make_dictionary(self, tweet_json):
        ...
        if ("media" in tweet_entities):
            ...
            tweet_media = []
        tweet_dict = {"postID": id, "text": tweet_text, "user": tweet_user, "coords": tweet_cordinates, "urls": tweet_urls,
                      "media": tweet_media}
        return tweet_dict
